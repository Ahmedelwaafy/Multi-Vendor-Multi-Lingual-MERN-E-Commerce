...
import { setVendorSession } from "@/app/Features/AuthenticationSlice";
...
export function Component() {
  ...
  const {
    ...
    async (data) => {
      ...
      //localStorage.setItem("VD", JSON.stringify(data?.data));
      console.log(data);

      Cookies.set("VT", data?.data?.vendor?._id);
      //dispatchRedux(setVendorData(data?.data));
      await new Promise<void>((resolve) => {
        setTimeout(() => {
          dispatchRedux(setVendorSession(data?.data?.vendor?._id));
          resolve();
        }, 500);
      });
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      postLoginData({
        api: import.meta.env.VITE_LOGIN_VENDOR,
        data,
      });
    } catch (error) {
      toast.error(
