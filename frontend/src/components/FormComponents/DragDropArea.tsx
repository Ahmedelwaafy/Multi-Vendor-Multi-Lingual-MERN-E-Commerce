...
function DragDropArea({ t }: DragDropAreaProps) {
  ...
  useEffect(() => {
    const images: File[] = [];
    validImages.map((img: imgType) => images.push(img.file));
  ...
  return (
    <div className=" w-full h-fit  p-5 ss:px-4  bg-gray-200 border-2 border-dashed  border-secondary rounded-md overflow-hidden ">
      {/**
       * //! --- draggable__area ---
       */}
      <div
        ...
        onDrop={onDrop}
        className="draggable__area flex flex-col items-center justify-center w-  min-h-[20px]"
      >
        ...
        {isDragging ? (
          ...
          <>
            ...
            <h4 className="mb-2 text-lg font-medium">{t("DragDropArea.or")}</h4>
            <button
              onClick={triggerFileInput}
              className="relative bg-secondary opacity20  px-3 py-1 text-lg rounded-md group text-background"
              type="button"
            >
        ...
        <div className="images__preview grid grid-cols-3 w-full mt-10 justify-items-center gap-10 md:grid-cols-2 ss:grid-cols-1">
          {validImages.map((img: imgType, i) => (
            <div key={i} className="w-fit h-fit relative group">
              <img
                className="w-full aspect-square object-cover rounded-md max-w-[160px]  max-h-[160px] group-hover:blur-[2px] duration-200 "
                src={img.url}
                alt={img.name}
              />
              <FontAwesomeIcon
